...
export class Hud {
  ...
  showStall(stall: Stall) { this.openStall = stall; this.renderPanel(stall); $('panel').hidden = false; }
  closePanel() { this.openStall = null; $('panel').hidden = true; }
  ...
  private renderPanel(s: Stall) {
    ...
    panel.innerHTML = `
      ...
      <div class="row"><button data-a="close">閉じる</button><button data-a="remove" class="danger">撤去 (+${Math.floor(def.cost / 2)})</button></div>`;
